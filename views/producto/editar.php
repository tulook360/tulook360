<div class="section-header">
    <div>
        <h1 class="section-title kalam">Editar Producto</h1>
        <p class="section-subtitle">Modificando: <b><?= htmlspecialchars($producto['pro_nombre']) ?></b></p>
    </div>
...
<div class="main-wrapper">
    ...
    <form action="<?= ruta_accion('producto', 'actualizar') ?>" method="POST" enctype="multipart/form-data" autocomplete="off" id="formProducto" class="form-content-box">
        <input type="hidden" name="id" value="<?= $producto['pro_id'] ?>">

        <div class="step-panel active-panel" id="step1">
            <div class="panel-header">
                <h3>Clasificación</h3>
                <p>Mueve el producto a otra categoría si deseas.</p>
            </div>
            
            <input type="hidden" name="tpro_id" id="inputCategoriaId" value="<?= $producto['tpro_id'] ?>">
            
            <div class="category-grid" id="gridCategorias">
                <?php foreach($listaCategorias as $cat): ?>
                    <?php $sel = ($cat['tpro_id'] == $producto['tpro_id']) ? 'selected' : ''; ?>
                    <div class="cat-card <?= $sel ?>" onclick="seleccionarCategoria(this, <?= $cat['tpro_id'] ?>)">
        ...
        <div class="step-panel" id="step2">
            <div class="panel-header">
                <h3>Información</h3>
                <p>Datos de identidad y stock.</p>
            </div>
            ...
            <div class="split-layout">
                <div class="upload-section">
                    <label class="photo-uploader" id="boxFotoUploader" for="inputFotos">
                        ...
                        <?php $fotosGuardadas = $producto['galeria'] ?? []; $conteo = count($fotosGuardadas); ?>
                        
                        <div class="upload-placeholder" id="placeholderGaleria" style="display: <?= $conteo > 0 ? 'none' : 'flex' ?>">
                            <div class="icon-upload"><i class="fa-solid fa-cloud-arrow-up"></i></div>
                            <strong>Fotos del Producto <span style="color:red">*</span></strong>
                            <span>Clic para subir (Máx 3)</span>
                        </div>

                        <div id="previewContainer" class="preview-grid <?= $conteo > 0 ? 'has-files' : '' ?>">
                            <?php foreach($fotosGuardadas as $foto): ?>
                                <div class="img-card" id="saved-<?= $foto['img_id'] ?>">
                                    <img src="<?= htmlspecialchars($foto['img_url']) ?>">
                                    <div class="btn-delete-img" onclick="borrarFotoServidor(event, <?= $foto['img_id'] ?>, '<?= $foto['img_url'] ?>')">
                    ...
                    <div style="text-align:center; margin-top:8px; color:#b2bec3; font-size:0.8rem;">
                        <span id="conteoTotal"><?= $conteo ?></span> / 3 Imágenes
                    </div>
                    <div id="errorFotos" class="error-msg" style="display:none; text-align:center;">Debes tener al menos 1 foto.</div>
                </div>
                ...
                <div class="inputs-section">
                    <div class="input-group-modern">
                        ...
                        <div class="input-icon-wrap">
                            ...
                            <input type="text" name="nombre" id="txtNombre" value="<?= htmlspecialchars($producto['pro_nombre']) ?>">
                        </div>
                    </div>

                    <div class="input-group-modern">
                        <label>Código / SKU <span style="color:red">*</span></label>
                        <div class="input-icon-wrap" style="position:relative;">
                            <i class="fa-solid fa-barcode input-icon"></i>
                            <input type="text" name="codigo" id="txtCodigo" value="<?= htmlspecialchars($producto['pro_codigo']) ?>">
                            <i class="fa-solid fa-arrows-rotate icon-inside-right" onclick="generarCodigo()"></i>
                        </div>
                    </div>
                    ...
                    <div class="row-inputs">
                        <div class="input-group-modern">
                            ...
                            <div class="input-icon-wrap">
                                ...
                                <input type="number" name="stock" id="txtStock" value="<?= $producto['pro_stock'] ?>">
                            </div>
                        </div>
                        <div class="input-group-modern">
                            ...
                            <div class="input-icon-wrap">
                                ...
                                <input type="number" step="0.01" name="costo" id="txtCosto" value="<?= $producto['pro_costo_compra'] ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="input-group-modern">
                        ...
                        <div class="input-icon-wrap">
                            ...
                            <textarea name="descripcion" id="txtDescripcion" rows="3"><?= htmlspecialchars($producto['pro_descripcion']) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="step-panel" id="step3">
            ...
            <div class="product-type-selector" id="boxTiposUso">
                <label class="type-card" id="cardVenta">
                    <input type="checkbox" name="venta" value="1" id="checkVenta" <?= $producto['pro_venta'] ? 'checked' : '' ?> onchange="toggleSections()">
                    <div class="type-icon"><i class="fa-solid fa-cash-register"></i></div>
                    <div class="type-info"><strong>Para Venta</strong><span>Cliente se lo lleva.</span></div>
                    <div class="check-circle"><i class="fa-solid fa-check"></i></div>
                </label>

                <label class="type-card" id="cardInsumo">
                    <input type="checkbox" name="insumo" value="1" id="checkInsumo" <?= $producto['pro_insumo'] ? 'checked' : '' ?> onchange="toggleSections()">
                    <div class="type-icon"><i class="fa-solid fa-flask"></i></div>
                    <div class="type-info"><strong>Es Insumo</strong><span>Uso interno.</span></div>
                    <div class="check-circle"><i class="fa-solid fa-check"></i></div>
                </label>
            </div>
            <div id="errorTiposUso"></div>

            <div id="sectionVenta" class="dynamic-box animate-pop">
                <div class="price-hero">
                    <label>Precio Venta ($) <span style="color:red">*</span></label>
                    <div class="price-input-wrapper">
                        <span class="currency-symbol">$</span>
                        <input type="number" step="0.01" name="precio" id="inputPrecioVenta" value="<?= $producto['pro_precio'] ?>">
                    </div>
                </div>
            </div>
                ...
                <div class="conversion-row">
                    <div class="conv-group">
                        ...
                        <select name="unidad" id="unidadCompra" onchange="actualizarFrases()">
                            <?php $uni = $producto['pro_unidad']; ?>
                            <option value="Unidad" <?= $uni=='Unidad'?'selected':''?>>Unidad</option>
                            <option value="Botella" <?= $uni=='Botella'?'selected':''?>>Botella</option>
                            <option value="Caja" <?= $uni=='Caja'?'selected':''?>>Caja</option>
                            <option value="Paquete" <?= $uni=='Paquete'?'selected':''?>>Paquete</option>
                            <option value="Galon" <?= $uni=='Galon'?'selected':''?>>Galón</option>
                        </select>
                    </div>
                    ...
                    <div class="conv-group">
                        ...
                        <div class="input-icon-wrap small-icon">
                            ...
                            <input type="number" step="0.01" name="contenido" id="inputContenido" value="<?= floatval($producto['pro_contenido']) ?>">
                        </div>
                    </div>
                    <div class="conv-group">
                        <label>Unidad Uso</label>
                        <select name="unidad_consumo" id="unidadConsumo" onchange="actualizarFrases()">
                            <?php $uc = $producto['pro_unidad_consumo']; ?>
                            <option value="ml" <?= $uc=='ml'?'selected':''?>>ml</option>
                            <option value="gr" <?= $uc=='gr'?'selected':''?>>gr</option>
                            <option value="oz" <?= $uc=='oz'?'selected':''?>>oz</option>
                            <option value="unidades" <?= $uc=='unidades'?'selected':''?>>unidades</option>
                            <option value="pares" <?= $uc=='pares'?'selected':''?>>pares</option>
                            <option value="aplicaciones" <?= $uc=='aplicaciones'?'selected':''?>>aplicaciones</option>
                        </select>
                    </div>
                </div>
                <p class="helper-text" id="lblEjemploConversion">Ej: 1 Botella contiene 1000 ml.</p>
            </div>
        </div>
    ...
    <div class="modal-container animate-pop">
        ...
        <div class="modal-content-inner">
            ...
            <div class="input-modal-wrap">
                <input type="text" id="txtNuevaCat" placeholder="Ej: Shampoos..." autocomplete="off">
            </div>
        </div>
...
<style>
    ...
    /* Agregamos .input-icon-wrap input para que afecte también al campo "Contiene" */
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        toggleSections();
        actualizarFrases();
        // Recalcular fotos existentes para la validación
        fotosExistentes = document.querySelectorAll('.img-card[id^="saved-"]').length;
        actualizarUI();
    });
    ...
    function cambiarPaso(dir) {
        if (dir === 1) {
            ...
            if (currentStep === 2) {
                ...
                if (fotosExistentes + fotosNuevas.length === 0) {
    ...
    function toggleSections() {
        ...
        document.getElementById('sectionPresentacion').style.display = 'block';
    ...
    let fotosExistentes = document.querySelectorAll('.img-card[id^="saved-"]').length;
    let fotosNuevas = [];
    const MAX_FOTOS = 3;

    inputFotos.addEventListener('change', function(e) {
        const files = Array.from(this.files);
        if (MAX_FOTOS - (fotosExistentes + fotosNuevas.length) <= 0) { mostrarAlerta("Límite", "Máximo 3 fotos."); return; }
        
        files.forEach(file => {
            if (['image/jpeg', 'image/png', 'image/jpg'].includes(file.type)) {
                if (fotosNuevas.length + fotosExistentes < MAX_FOTOS) fotosNuevas.push(file);
            }
        });
    ...
    function renderizarNuevas() {
        const nuevas = previewCont.querySelectorAll('.new-preview');
        nuevas.forEach(n => n.remove());

        fotosNuevas.forEach((file, index) => {
            const reader = new FileReader();
            const div = document.createElement('div'); div.className = 'img-card new-preview';
            const img = document.createElement('img'); img.className = 'thumb-img';
            const btn = document.createElement('div'); btn.className = 'btn-delete-img';
            btn.innerHTML = '<i class="fa-solid fa-xmark"></i>';
            btn.onclick = (e) => { e.preventDefault(); e.stopPropagation(); borrarNueva(index); };
    ...
    function borrarNueva(index) {
        ...
        renderizarNuevas(); actualizarUI();
    }

    window.borrarFotoServidor = function(e, imgId, url) {
        e.preventDefault(); e.stopPropagation();
        const modal = document.getElementById('modalConfirm');
        document.getElementById('modalTitle').innerText = "Eliminar Foto";
        document.getElementById('modalText').innerText = "¿Eliminar permanentemente?";
        const btnCancel = document.getElementById('btnModalCancelar');
        ...
        newBtn.onclick = function() {
            ...
            fetch('<?= ruta_accion("producto", "borrar_foto", [], false) ?>', { method: 'POST', body: fd })
            .then(r => r.json()).then(d => {
                if(d.success) {
                    document.getElementById('saved-' + imgId).remove();
                    fotosExistentes--; actualizarUI();
                } else {
                    mostrarAlerta('Error', d.message || 'No se pudo borrar');
                }
            });
        };
        modal.classList.add('active');
    ...
    function actualizarUI() {
        let total = fotosExistentes + fotosNuevas.length;
        counter.innerText = total;
        if(total > 0) { 
            placeholder.style.display = 'none'; 
            previewCont.classList.add('has-files'); 
            document.getElementById('boxFotoUploader').classList.remove('input-error'); // Quitar error visual
    ...
    function checkLimit() { actualizarUI(); }
</script>