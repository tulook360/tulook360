...
<style>
    /* --- ESTILOS IDENTICOS AL CREAR (PREMIUM) --- */
    ...
    /* Nota: Si tiene imagen, mostramos la img y ocultamos el placeholder */
...
<div class="section-header">
    <div>
        <h1 class="section-title kalam">Editar Sucursal</h1>
        <p class="section-subtitle">Modificando: <b><?= htmlspecialchars($sucursal['suc_nombre']) ?></b></p>
    </div>
...
<div class="main-wrapper">
    ...
    <div class="stepper-container">
        ...
        <div class="step-item active" id="stepIndicator1"><div class="step-circle"><i class="fa-solid fa-store"></i></div><span class="step-label">Identidad</span></div>
        <div class="step-item" id="stepIndicator2"><div class="step-circle"><i class="fa-solid fa-address-book"></i></div><span class="step-label">Contacto</span></div>
        <div class="step-item" id="stepIndicator3"><div class="step-circle"><i class="fa-solid fa-map-location-dot"></i></div><span class="step-label">Ubicación</span></div>
        <div class="step-item" id="stepIndicator4"><div class="step-circle"><i class="fa-regular fa-clock"></i></div><span class="step-label">Horarios</span></div>
    </div>

    <form action="<?= ruta_accion('sucursal', 'actualizar') ?>" method="POST" enctype="multipart/form-data" autocomplete="off" id="formSucursal" class="form-content-box">
        
        <input type="hidden" name="id" value="<?= $sucursal['suc_id'] ?>">

        <div class="step-panel active-panel" id="step1">
            <div class="panel-header"><h3>¿Cómo se ve tu local?</h3><p>Modifica la imagen o el nombre principal.</p></div>
            
            <div class="input-group-modern" style="text-align:center;">
                <label>Foto de Fachada</label>
                
                <?php 
                    $tieneFoto = !empty($sucursal['suc_foto']);
                    $srcFoto = $tieneFoto ? $sucursal['suc_foto'] : '';
                    $claseFoto = $tieneFoto ? 'has-image' : '';
                ?>

                <label class="photo-upload-zone <?= $claseFoto ?>" id="photoZone">
                    <input type="file" name="foto" id="inputFoto" accept="image/*" style="display:none;">
                    
                    <img id="imgPreview" src="<?= $srcFoto ?>" alt="Preview">
                    
                    <div class="upload-placeholder">
                        <div class="icon-box-circle">
                            <i class="fa-solid fa-camera" style="font-size: 1.6rem; color:var(--color-primario);"></i>
                        </div>
                        <div style="font-weight:700; color:#2d3436; font-size:1.1rem; margin-bottom:5px;">Cambiar foto</div>
                        <small style="font-size: 0.85rem; color:#b2bec3;">Clic para subir nueva imagen</small>
                    </div>
                </label>
            </div>
            ...
            <div class="input-group-modern">
                ...
                <input type="text" name="nombre" id="txtNombre" value="<?= htmlspecialchars($sucursal['suc_nombre']) ?>" placeholder="Ej: Matriz - Centro Histórico">
            </div>
        </div>

        <div class="step-panel" id="step2">
            <div class="panel-header"><h3>Datos de Contacto</h3><p>Información para el cliente.</p></div>
            
            <div class="input-group-modern">
                <label>Teléfono / WhatsApp</label>
                <input type="text" name="telefono" id="txtTelefono" value="<?= htmlspecialchars($sucursal['suc_telefono']) ?>" placeholder="Ej: 0991234567">
            </div>

            <div class="input-group-modern">
                <label>Correo Electrónico (Opcional)</label>
                <input type="email" name="correo" value="<?= htmlspecialchars($sucursal['suc_correo']) ?>" placeholder="sucursal@tulook.com">
            </div>
        ...
        <div class="step-panel" id="step3">
            <div class="panel-header"><h3>Ubicación Geográfica</h3><p>Modifica el punto en el mapa.</p></div>
            ...
            <input type="hidden" name="latitud" id="latitud" value="<?= $sucursal['suc_latitud'] ?>">
            <input type="hidden" name="longitud" id="longitud" value="<?= $sucursal['suc_longitud'] ?>">

            <div class="row-inputs" style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-top:20px;">
                <div class="input-group-modern">
                    <label>Dirección <span style="color:red">*</span></label>
                    <input type="text" name="direccion" id="txtDireccion" value="<?= htmlspecialchars($sucursal['suc_direccion']) ?>" placeholder="Ej: Av. Amazonas y NNUU">
                </div>
                <div class="input-group-modern">
                    <label>Referencia</label>
                    <input type="text" name="referencia" value="<?= htmlspecialchars($sucursal['suc_referencia']) ?>" placeholder="Frente al parque...">
                </div>
            </div>
        </div>

        <div class="step-panel" id="step4">
            <div class="panel-header"><h3>Horarios de Atención</h3><p>Configura la disponibilidad semanal.</p></div>

            <div style="text-align:right;">
                <button type="button" class="btn-replicar" onclick="replicarHorario()">
                    <i class="fa-solid fa-copy"></i> Copiar Lunes a todos
                </button>
            </div>

            <div class="horarios-wrapper">
                <?php 
                // 1. MAPEAR LOS DATOS QUE VIENEN DE LA BD
                // Asumimos que en la BD 'sh_dia' ya es un número (1, 2, 3...)
                $mapaHorarios = [];
                if(isset($horariosDb) && is_array($horariosDb)){
                    foreach($horariosDb as $h){
                        // Si ya hiciste la migración SQL, sh_dia es un número.
                        // Si aún son textos en la BD, este mapeo temporal fallará hasta que corras el SQL.
                        $mapaHorarios[$h['sh_dia']] = $h; 
                    }
                }

                // 2. DEFINIR NOMBRES PARA MOSTRAR
                $nombresDias = [
                ...
                for ($i = 1; $i <= 7; $i++): 
                    ...
                    // Buscamos en el mapa usando el índice NUMÉRICO ($i)
                    $datosDia = $mapaHorarios[$i] ?? null;
                    
                    // Valores
                    $valApertura = $datosDia ? date('H:i', strtotime($datosDia['sh_apertura'])) : '09:00';
                    $valCierre   = $datosDia ? date('H:i', strtotime($datosDia['sh_cierre'])) : '18:00';
                    $esDescanso  = ($datosDia && $datosDia['sh_es_descanso'] == 1) ? true : false;
                ?>
                <div class="horario-row" id="row_<?= $i ?>">
                    ...
                    <input type="time" class="time-input inp-apertura" name="horarios[<?= $i ?>][apertura]" value="<?= $valApertura ?>">
                    <input type="time" class="time-input inp-cierre" name="horarios[<?= $i ?>][cierre]" value="<?= $valCierre ?>">
                    
                    <div style="text-align:center;">
                        <label class="switch">
                            <input type="checkbox" class="chk-descanso" name="horarios[<?= $i ?>][es_descanso]" value="1" 
                                <?= $esDescanso ? 'checked' : '' ?> 
                                onchange="toggleDescanso(<?= $i ?>)">
...
<script>
    ...
    function cambiarPaso(dir) {
        if (dir === 1) {
            if (currentStep === 1) {
                ...
                if (!nombre) { mostrarNotificacion("Falta información", "El nombre es obligatorio.", "danger"); return; }
            }
            if (currentStep === 3) {
                 const direccion = document.getElementById('txtDireccion').value.trim();
                 if (!direccion) { mostrarNotificacion("Falta dirección", "La dirección es obligatoria.", "danger"); return; }
            }
        }
    ...
    // Inicializar estados al cargar (Bucle 1 al 7)
    ...
    // --- MAPA (CARGAR COORDENADAS GUARDADAS) ---
    // Usamos los valores de los inputs hidden o defaults si están vacíos
    let latSaved = document.getElementById('latitud').value;
    let lngSaved = document.getElementById('longitud').value;

    let latDefault = latSaved ? parseFloat(latSaved) : -0.180653; 
    let lngDefault = lngSaved ? parseFloat(lngSaved) : -78.467834;
